create or replace procedure altera_status_analise_gpt(	nr_atendimento_p	IN gpt_hist_analise_plano.nr_atendimento%type,
					cd_pessoa_fisica_p	IN gpt_hist_analise_plano.cd_pessoa_fisica%type,
					cd_resp_analise_p	IN gpt_hist_analise_plano.cd_resp_analise%type,
					ie_tipo_usuario_p	IN gpt_hist_analise_plano.ie_tipo_usuario%type,
					ie_acao_p		IN varchar2,
					nm_usuario_p		IN varchar2,
					ds_justificativa_p	IN varchar2 default null,
					ie_lib_farm_p		IN varchar2 default null,
					ie_retorno_cpoe_p	IN varchar2 default null,
					qt_horas_quimio_p	IN number default 0,
					nr_prescricao_p	    IN prescr_medica.nr_prescricao%type default null) is 
...
/*So acessa o bloco if se for executado da funcao Analise da Prescricao de Oncologia - APO ou Quimioterapia*/
...
if	(nr_sequencia_w > 0 and ((nm_resp_analise_w <> nm_usuario_p) and (desfazer_inicio_analise_w <> 'S'))) then
	...
	begin
		if	((nvl(nr_sequencia_w,0) = 0) and ((upper(ie_acao_p) = 'I') or (ie_retorno_cpoe_p = 'S'))) then
			...
			begin
			/*So acessa o bloco if se for executado da funcao Analise da Prescricao de Oncologia - APO ou Quimioterapia*/
				if (obter_funcao_ativa = 3130 or obter_funcao_ativa = 476)	then
					...
					liberar_prescricao_enf_html5(nr_prescricao_p,nr_atendimento_p,'N',Obter_perfil_Ativo,nm_usuario_p,'N',ds_erro_prescr_w,nr_prescr_erro_w);
					end if;
